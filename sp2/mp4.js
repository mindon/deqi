// node_modules/.deno/@remotion+media-parser@4.0.421/node_modules/@remotion/media-parser/dist/esm/index.mjs
...
var createAacCodecPrivate = ({ audioObjectType, sampleRate, channelConfiguration, codecPrivate: codecPrivate2 }) => {
...
var parseFtyp = ({ iterator, size, offset }) => {
...
var webmPattern = new Uint8Array([
  26,
  69,
  223,
  163
...
var isPdf = (data) => {
  ...
  return matchesPattern(pdfPattern)(data.subarray(0, 4)) ? {
    type: "pdf"
  } : null;
};
...
var bufferManager = ({ initialData, maxBytes, counter, logLevel }) => {
...
var getArrayBufferIterator = ({ initialData, maxBytes, logLevel }) => {
  const counter = makeOffsetCounter(0);
  const { getUint8Array, getView, addData, destroy, removeBytesRead, skipTo, replaceData } = bufferManager({
    initialData,
    maxBytes,
    counter,
    logLevel
...
var toUnixTimestamp = (value) => {
  ...
  const baseDate = /* @__PURE__ */ new Date("1904-01-01T00:00:00Z");
  return Math.floor(value + baseDate.getTime() / 1e3) * 1e3;
};
var parseMvhd = ({ iterator, offset, size }) => {
...
var createSpsPpsData = (avc1Profile) => {
  return combineUint8Arrays([
    ...
    [
      ...
      77,
      88
    ].some((b) => avc1Profile.sps.spsData.profile === b) ? null : new Uint8Array([
      253,
      248,
      248,
      0
...
var addAvcProfileToTrack = (track, avc1Profile) => {
  ...
  return {
    ...
    codecData: {
      type: "avc-sps-pps",
      data: createSpsPpsData(avc1Profile)
    },
    description: void 0
  };
};
var registerVideoTrack = async ({ track, container, logLevel, onVideoTrack, registerVideoSampleCallback, tracks: tracks2 }) => {
...
var registerAudioTrack = async ({ track, container, tracks: tracks2, logLevel, onAudioTrack, registerAudioSampleCallback }) => {
  if (tracks2.getTracks().find((t) => t.trackId === track.trackId)) {
    Log.trace(logLevel, `Track ${track.trackId} already registered, skipping`);
    return null;
  }
...
var registerVideoTrackWhenProfileIsAvailable = ({ state, track, container }) => {
...
var parseElst = ({ iterator, size, offset }) => {
...
var processDescriptor = ({ iterator }) => {
...
var parseEsds = ({ data, size, fileOffset }) => {
...
var getMoovBoxFromState = ({ structureState: structureState2, isoState, mp4HeaderSegment, mayUsePrecomputed }) => {
  ...
  const structure = structureState2.getIsoStructure();
...
var findTrackStartTimeInSeconds = ({ movieTimeScale, trakBox }) => {
...
var findTrackMediaTimeOffsetInTrackTimescale = ({ trakBox }) => {
  const elstBox = getElstBox(trakBox);
  if (!elstBox) {
    return 0;
  }
...
var makeAviAudioTrack = ({ strf, index }) => {
...
var makeAviVideoTrack = ({ strh, strf, index }) => {
...
var getTracksFromAvi = (structure, state) => {
  ...
  for (const box of boxes) {
    ...
    if (strf.type === "strf-box-video") {
      tracks2.push(addAvcProfileToTrack(makeAviVideoTrack({
        strh,
        strf,
        index: i
      }), state.riff.getAvcProfile()));
    } else if (strh.fccType === "auds") {
      tracks2.push(makeAviAudioTrack({
        strf,
        index: i
      }));
    } else {
...
var getMatroskaVideoCodecEnum = ({ codecSegment: codec }) => {
...
var getMatroskaVideoCodecString = ({ track, codecSegment: codec }) => {
  if (codec.value === "V_VP8") {
    return "vp8";
  }
  if (codec.value === "V_VP9") {
...
var getMatroskaAudioCodecEnum = ({ track }) => {
  const codec = getCodecSegment(track);
  if (!codec) {
...
var getTrack = ({ timescale: timescale2, track }) => {
  ...
  if (trackTypeToString(trackType2.value.value) === "video") {
    ...
    const codecData = codecPrivate2 === null ? null : codecEnum === "h264" ? {
      type: "avc-sps-pps",
      data: codecPrivate2
    } : codecEnum === "av1" ? {
      type: "av1c-data",
  ...
  if (trackTypeToString(trackType2.value.value) === "audio") {
    ...
    return {
      ...
      codecData: codecPrivate2 ? codecString === "opus" ? {
...
var getTracksFromMatroska = ({ structureState: structureState2, webmState: webmState2 }) => {
  const structure = structureState2.getMatroskaStructure();
  ...
  for (const trackEntrySegment of tracksSegment.value) {
    ...
    const track = getTrack({
      ...
      timescale: webmState2.getTimescale()
    });
    if (!track) {
      continue;
    }
    if (track.codec === NO_CODEC_PRIVATE_SHOULD_BE_DERIVED_FROM_SPS) {
      const avc = webmState2.getAvcProfileForTrackNumber(track.trackId);
      if (avc) {
        resolvedTracks.push({
          ...track,
...
var matroskaHasTracks = ({ structureState: structureState2, webmState: webmState2 }) => {
  const structure = structureState2.getMatroskaStructure();
  const mainSegment = getMainSegment(structure.boxes);
  if (!mainSegment) {
    return false;
...
var getTracksFromIsoBaseMedia = ({ mayUsePrecomputed, structure, isoState, m3uPlaylistContext }) => {
...
var getCodecSpecificatorFromEsdsBox = ({ child }) => {
...
var calculateFps = ({ sttsBox, timeScale, durationInSamples }) => {
  let totalSamples = 0;
...
var applyAspectRatios = ({ dimensions, sampleAspectRatio, displayAspectRatio }) => {
...
var getActualDecoderParameters = ({ audioCodec, codecPrivate: codecPrivate2, numberOfChannels, sampleRate }) => {
  if (audioCodec !== "aac") {
    return {
      numberOfChannels,
      sampleRate,
...
var makeBaseMediaTrack = (trakBox, startTimeInSeconds) => {
  ...
  if (trakBoxContainsAudio(trakBox)) {
    ...
    return {
      ...
      description: actual.codecPrivate?.data ?? void 0,
  ...
  const track = {
    ...
    type: "video",
    trackId: tkhdBox.trackId,
    description: videoDescriptors ?? void 0,
    originalTimescale: timescaleAndDuration.timescale,
    codec,
...
var parseMdhd = ({ data, size, fileOffset }) => {
  const version = data.getUint8();
  data.discard(3);
...
var parseHdlr = ({ iterator, size, offset }) => {
...
var parseTfraBox = ({ iterator, size, offset }) => {
  const box = iterator.startBox(size - 8);
  const version = iterator.getUint8();
  iterator.discard(3);
...
var parseMoov = async ({ offset, size, onlyIfMoovAtomExpected, iterator, logLevel, contentLength }) => {
...
var parseTrex = ({ iterator, offset, size }) => {
  const box = iterator.startBox(size - 8);
  const version = iterator.getUint8();
  iterator.discard(3);
  const trackId = iterator.getUint32();
...
var parseAv1C = ({ data, size }) => {
  return {
    type: "av1C-box",
    privateData: data.getSlice(size - 8)
  };
};
var parseAvcc = ({ data, size }) => {
...
var parseColorParameterBox = ({ iterator, size }) => {
...
var parseCtts = ({ iterator, offset, size }) => {
  const version = iterator.getUint8();
...
var parseHvcc = ({ data, size, offset }) => {
  const privateData = data.getSlice(size - 8);
...
var parseKeys = ({ iterator, offset, size }) => {
  const box = iterator.startBox(size - 8);
  const version = iterator.getUint8();
  iterator.discard(3);
  const entryCount = iterator.getUint32();
...
var parseMebx = async ({ offset, size, iterator, logLevel, contentLength }) => {
  iterator.discard(6);
...
var parsePasp = ({ iterator, offset, size }) => {
...
var parseStco = ({ iterator, offset, size, mode64Bit }) => {
  const version = iterator.getUint8();
  if (version !== 0) {
...
var parseStsc = ({ iterator, offset, size }) => {
  const version = iterator.getUint8();
  if (version !== 0) {
    throw new Error(`Unsupported STSD version ${version}`);
  }
...
var parseStsd = async ({ offset, size, iterator, logLevel, contentLength }) => {
  const version = iterator.getUint8();
  if (version !== 0) {
    throw new Error(`Unsupported STSD version ${version}`);
  }
...
var parseStss = ({ iterator, offset, boxSize }) => {
  const version = iterator.getUint8();
  if (version !== 0) {
  ...
  const flags = iterator.getSlice(3);
  const sampleCount = iterator.getUint32();
  const sampleNumber = /* @__PURE__ */ new Set();
...
var parseStsz = ({ iterator, offset, size }) => {
  const version = iterator.getUint8();
  if (version !== 0) {
    throw new Error(`Unsupported STSD version ${version}`);
  }
...
var parseStts = ({ data, size, fileOffset }) => {
...
var parseTfdt = ({ iterator, size, offset }) => {
  const version = iterator.getUint8();
  iterator.discard(3);
...
var getTfhd = ({ iterator, offset, size }) => {
  const version = iterator.getUint8();
  const flags = iterator.getUint24();
  const trackId = iterator.getUint32();
...
var applyRotation = ({ matrix, width, height }) => {
...
var parseTkhd = ({ iterator, offset, size }) => {
  const version = iterator.getUint8();
  iterator.discard(3);
  const creationTime = version === 1 ? iterator.getUint64() : iterator.getUint32();
  const modificationTime = version === 1 ? iterator.getUint64() : iterator.getUint32();
...
var parseTrak = async ({ size, offsetAtStart, iterator, logLevel, contentLength }) => {
  const children = await getIsoBaseMediaChildren({
    onlyIfMoovAtomExpected: null,
    size: size - 8,
    iterator,
...
var parseTrun = ({ iterator, offset, size }) => {
  const version = iterator.getUint8();
  if (version !== 0 && version !== 1) {
...
var processBox = async ({ iterator, logLevel, onlyIfMoovAtomExpected, onlyIfMdatAtomExpected, contentLength }) => {
  ...
  if (boxType === "trak") {
    ...
    const editList = findTrackStartTimeInSeconds({
      movieTimeScale,
      trakBox
    });
...
var getIsoBaseMediaChildren = async ({ size, iterator, logLevel, onlyIfMoovAtomExpected, contentLength }) => {
  const boxes = [];
...
var processIsoFormatBox = async ({ iterator, logLevel, contentLength }) => {
  const fileOffset = iterator.counter.getOffset();
  const bytesRemaining = iterator.bytesRemaining();
...
var parseIsoFormatBoxes = async ({ maxBytes, logLevel, iterator, contentLength }) => {
  const samples = [];
  const initialOffset = iterator.counter.getOffset();
  while (iterator.bytesRemaining() > 0 && iterator.counter.getOffset() - initialOffset < maxBytes) {
...
var getPoc = (iterator, sps, avcState2, isReferencePicture) => {
  const { pic_order_cnt_type, log2_max_pic_order_cnt_lsb_minus4 } = sps;
  if (pic_order_cnt_type !== 0) {
    return null;
  }
  const prevPicOrderCntLsb = avcState2.getPrevPicOrderCntLsb();
  const prevPicOrderCntMsb = avcState2.getPrevPicOrderCntMsb();
  ...
  if (isReferencePicture) {
    avcState2.setPrevPicOrderCntLsb(pic_order_cnt_lsb);
    avcState2.setPrevPicOrderCntMsb(picOrderCntMsb);
  }
  return poc;
};
...
var inspect = (buffer, avcState2) => {
  ...
  if (type === 7) {
    ...
    avcState2.setSps(data);
    if (isReferencePicture) {
      avcState2.setPrevPicOrderCntLsb(0);
      avcState2.setPrevPicOrderCntMsb(0);
    }
    return {
      spsData: data,
      sps,
  ...
  if (type === 5) {
    ...
    let poc = null;
    if (pic_order_cnt_type === 0) {
      poc = getPoc(iterator, sps, avcState2, isReferencePicture);
    }
    iterator.stopReadingBits();
    return {
      type: "keyframe",
...
var parseAvc = (buffer, avcState2) => {
  ...
  for (let i = 0; i < buffer.length; i++) {
    ...
    if (zeroesInARow >= 2 && val === 1) {
      zeroesInARow = 0;
      const info = inspect(buffer.slice(i + 1, i + 100), avcState2);
      if (info) {
        infos.push(info);
...
var addAvcToTrackAndActivateTrackIfNecessary = async ({ partialVideoSample, codec, structureState: structureState2, webmState: webmState2, trackNumber: trackNumber2, logLevel, callbacks, onVideoTrack, avcState: avcState2 }) => {
  ...
  const parsed = parseAvc(partialVideoSample.data, avcState2);
  for (const parse of parsed) {
    if (parse.type === "avc-profile") {
      webmState2.setAvcProfileForTrackNumber(trackNumber2, parse);
...
var getSampleFromBlock = async ({ ebml, webmState: webmState2, offset, structureState: structureState2, callbacks, logLevel, onVideoTrack, avcState: avcState2 }) => {
  const iterator = getArrayBufferIterator({
  ...
  const { codec, trackTimescale } = webmState2.getTrackInfoByNumber(trackNumber2);
  const clusterOffset = webmState2.getTimestampOffsetForByteOffset(offset);
  const timescale2 = webmState2.getTimescale();
  if (clusterOffset === void 0) {
  ...
  const timecodeInMicroseconds = timecodeInNanoSeconds / 1e3;
  if (!codec) {
...
var postprocessEbml = async ({ offset, ebml, statesForProcessing: { webmState: webmState2, callbacks, logLevel, onAudioTrack, onVideoTrack, structureState: structureState2, avcState: avcState2 } }) => {
  if (ebml.type === "TimestampScale") {
    webmState2.setTimescale(ebml.value.value);
  }
  if (ebml.type === "Tracks") {
    callbacks.tracks.setIsDone(logLevel);
  }
  if (ebml.type === "TrackEntry") {
    webmState2.onTrackEntrySegment(ebml);
    const track = getTrack({
      track: ebml,
      timescale: webmState2.getTimescale()
    });
  ...
  if (ebml.type === "Timestamp") {
    webmState2.setTimestampOffset(offset, ebml.value.value);
  }
...
var IsAnImageError = class _IsAnImageError extends Error {
  imageType;
  dimensions;
  mimeType;
  sizeInBytes;
  ...
  constructor({ dimensions, imageType, message, mimeType, sizeInBytes, fileName }) {
    ...
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, _IsAnImageError);
    }
  }
};
var IsAPdfError = class _IsAPdfError extends Error {
  mimeType;
  sizeInBytes;
  fileName;
  constructor({ message, mimeType, sizeInBytes, fileName }) {
    ...
    this.fileName = fileName;
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, _IsAPdfError);
    }
  }
};
var IsAnUnsupportedFileTypeError = class _IsAnUnsupportedFileTypeError extends Error {
  mimeType;
  sizeInBytes;
  fileName;
  constructor({ message, mimeType, sizeInBytes, fileName }) {
    ...
    this.fileName = fileName;
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, _IsAnUnsupportedFileTypeError);
    }
  }
};
var MediaParserAbortError = class extends Error {
  constructor(message) {
    super(message);
...
var MediaParserEmitter = class {
  ...
  dispatchResume = () => {
    this.readyPromise = this.readyPromise.then(() => {
      this.dispatchEvent("resume", void 0);
    });
  };
...
var mediaParserController = () => {
  ...
  return {
    ...
    getSeekingHints: getSeekingHints2,
    _internals: {
...
var getM3uStreams = ({ structure, originalSrc, readerInterface }) => {
  if (structure === null || structure.type !== "m3u") {
    return null;
  }
  const boxes = [];
...
var areSamplesComplete = ({ moofBoxes, tfraBoxes }) => {
...
var getSamplesFromMoof = ({ moofBox, trackId, trexBoxes }) => {
...
var collectSamplePositionsFromMoofBoxes = ({ moofBoxes, tkhdBox, isComplete, trexBoxes }) => {
...
var getSamplePositions = ({ stcoBox, stszBox, stscBox, stssBox, sttsBox, cttsBox }) => {
  const sttsDeltas = [];
...
var getGroupedSamplesPositionsFromMp4 = ({ trakBox, bigEndian }) => {
...
var getSamplePositionsFromTrack = ({ trakBox, moofBoxes, moofComplete, trexBoxes }) => {
  const tkhdBox = getTkhdBox(trakBox);
  if (!tkhdBox) {
    throw new Error("Expected tkhd box in trak box");
  }
...
var getAllPlaylists = ({ structure, src }) => {
...
var getUnroundedMpegFrameLength = ({ samplesPerFrame, bitrateKbit, samplingFrequency: samplingFrequency2, padding, layer }) => {
  if (layer === 1) {
    throw new Error("MPEG Layer I is not supported");
  }
  return samplesPerFrame / 8 * bitrateKbit / samplingFrequency2 * 1e3 + (padding ? layer === 1 ? 4 : 1 : 0);
};
var getAverageMpegFrameLength = ({ samplesPerFrame, bitrateKbit, samplingFrequency: samplingFrequency2, layer }) => {
...
var getMpegFrameLength = ({ samplesPerFrame, bitrateKbit, samplingFrequency: samplingFrequency2, padding, layer }) => {
...
var getSamplesPerMpegFrame = ({ mpegVersion, layer }) => {
  if (mpegVersion === 1) {
    if (layer === 1) {
      return 384;
    }
...
var getDurationFromMp3Xing = ({ xingData, samplesPerFrame }) => {
...
var getDurationFromMatroska = (segments) => {
  ...
  return duration2.value.value / timestampScale2.value.value * 1e3;
};
...
var parseIsoMetaBox = (meta, trackId) => {
  ...
  for (let i = 0; i < ilstBox.entries.length; i++) {
    ...
    if (ilstEntry.value.type !== "unknown") {
      const value = typeof ilstEntry.value.value === "string" && ilstEntry.value.value.endsWith("\0") ? ilstEntry.value.value.slice(0, -1) : ilstEntry.value.value;
      entries.push({
        key: keysEntry.value,
        value,
        trackId
...
var removeEndZeroes = (value) => {
  return value.endsWith("\0") ? removeEndZeroes(value.slice(0, -1)) : value;
};
...
var getSeekingByteForAac = ({ time, seekingHints }) => {
  let bestAudioSample;
...
var getSeekingByteForFlac = ({ time, seekingHints }) => {
  let bestAudioSample;
  for (const hint of seekingHints.audioSampleMap) {
    if (hint.timeInSeconds > time) {
      continue;
...
var findKeyframeBeforeTime = ({ samplePositions, time, timescale: timescale2, mediaSections, logLevel, startInSeconds }) => {
  let videoByte = 0;
  let videoSample = null;
  for (const sample of samplePositions) {
    const ctsInSeconds = sample.timestamp / timescale2 + startInSeconds;
    const dtsInSeconds = sample.decodingTimestamp / timescale2 + startInSeconds;
...
var isByteInMediaSection = ({ position, mediaSections }) => {
...
var getCurrentMediaSection = ({ offset, mediaSections }) => {
  for (const section of mediaSections) {
...
var getSamplePositionBounds = (samplePositions, timescale2) => {
  let min = Infinity;
  let max = -Infinity;
...
var findBestSegmentFromTfra = ({ mfra, time, firstTrack, timescale: timescale2 }) => {
  ...
  for (const segment of tfra.entries) {
    if (segment.time / timescale2 <= time) {
      bestSegment = segment;
    }
  }
  if (!bestSegment) {
...
var getSeekingByteFromFragmentedMp4 = async ({ info, time, logLevel, currentPosition, isoState, tracks: tracks2, isLastChunkInPlaylist, structure, mp4HeaderSegment }) => {
  const firstVideoTrack = tracks2.find((t) => t.type === "video");
...
var getSeekingByteFromIsoBaseMedia = ({ info, time, logLevel, currentPosition, isoState, m3uPlaylistContext, structure }) => {
  const tracks2 = getTracksFromIsoBaseMedia({
    isoState,
    m3uPlaylistContext,
    structure,
...
var clearM3uStateInPrepareForSeek = ({ m3uState: m3uState2, logLevel }) => {
  const selectedPlaylists = m3uState2.getSelectedPlaylists();
  for (const playlistUrl of selectedPlaylists) {
    const streamRun = m3uState2.getM3uStreamRun(playlistUrl);
    if (streamRun) {
      streamRun.abort();
    }
    Log.trace(logLevel, "Clearing M3U stream run for", playlistUrl);
    m3uState2.setM3uStreamRun(playlistUrl, null);
  }
  m3uState2.clearAllChunksProcessed();
  m3uState2.sampleSorter.clearSamples();
};
var getSeekingByteForM3u8 = ({ time, currentPosition, m3uState: m3uState2, logLevel }) => {
  clearM3uStateInPrepareForSeek({
    m3uState: m3uState2,
    logLevel
  });
  const selectedPlaylists = m3uState2.getSelectedPlaylists();
  for (const playlistUrl of selectedPlaylists) {
    m3uState2.setSeekToSecondsToProcess(playlistUrl, {
      targetTime: time
    });
  }
  return {
...
var getApproximateByteFromBitrate = ({ mp3BitrateInfo, timeInSeconds, mp3Info, mediaSection, contentLength }) => {
  if (mp3BitrateInfo.type === "variable") {
    return null;
  }
  const samplesPerFrame = getSamplesPerMpegFrame({
...
var getByteFromObservedSamples = ({ info, timeInSeconds }) => {
  let bestAudioSample;
...
var getSeekPointInBytes = ({ fileSize, percentBetween0And100, tableOfContents }) => {
...
var getTimeFromPosition = ({ position, fileSize, tableOfContents, durationInSeconds }) => {
...
var getSeekPointFromXing = ({ timeInSeconds, xingData, mp3Info }) => {
  const samplesPerFrame = getSamplesPerMpegFrame({
    layer: mp3Info.layer,
    mpegVersion: mp3Info.mpegVersion
  });
...
var getSeekingByteForMp3 = ({ time, info }) => {
...
var getSeekingByteForRiff = async ({ info, time, riffState, avcState: avcState2 }) => {
...
var convertAudioOrVideoSampleToWebCodecsTimestamps = ({ sample, timescale: timescale2 }) => {
  if (timescale2 === WEBCODECS_TIMESCALE) {
    return sample;
  }
  const { decodingTimestamp: dts, timestamp } = sample;
  return {
    decodingTimestamp: fixFloat(dts * (WEBCODECS_TIMESCALE / timescale2)),
    timestamp: fixFloat(timestamp * (WEBCODECS_TIMESCALE / timescale2)),
    duration: sample.duration === void 0 ? void 0 : fixFloat(sample.duration * (WEBCODECS_TIMESCALE / timescale2)),
    data: sample.data,
    type: sample.type,
...
var MPEG_TIMESCALE = 9e4;
var handleAvcPacket = async ({ streamBuffer, programId, offset, sampleCallbacks, logLevel, onVideoTrack, transportStream, makeSamplesStartAtZero, avcState: avcState2 }) => {
  const avc = parseAvc(streamBuffer.getBuffer(), avcState2);
...
var getSeekingByteFromWav = ({ info, time }) => {
  const bytesPerSecond = info.sampleRate * info.blockAlign;
  const durationInSeconds = info.mediaSection.size / bytesPerSecond;
  const timeRoundedDown = Math.floor(Math.min(time, durationInSeconds - 1e-7) * WAVE_SAMPLES_PER_SECOND) / WAVE_SAMPLES_PER_SECOND;
...
var toSeconds = (timeInTimescale, track) => {
  return timeInTimescale / track.timescale * 1e3;
};
var findBiggestCueBeforeTime = ({ cues, time, track }) => {
...
var findKeyframeBeforeTime2 = ({ keyframes, time }) => {
...
var getByteFromCues = ({ cuesResponse, time, info, logLevel }) => {
  if (!cuesResponse) {
...
var getSeekingByteFromMatroska = async ({ time, webmState: webmState2, info, logLevel, mediaSection }) => {
  if (!info.track) {
  ...
  const cuesResponse = info.loadedCues ?? await webmState2.cues.getLoadedCues();
  ...
  const byteFromFirstMediaSection = webmState2.getFirstCluster()?.start ?? null;
...
var getLastKeyFrameBeforeTimeInSeconds = ({ observedPesHeaders, timeInSeconds, ptsStartOffset }) => {
  return observedPesHeaders.findLast((k) => (k.pts - ptsStartOffset) / MPEG_TIMESCALE <= timeInSeconds);
};
var getSeekingByte = ({ info, time, logLevel, currentPosition, isoState, transportStream, webmState: webmState2, mediaSection, m3uPlaylistContext, structure, riffState, m3uState: m3uState2, avcState: avcState2 }) => {
...
var getSeekingHintsForAac = ({ aacState: aacState2, samplesObserved }) => {
  return {
    type: "aac-seeking-hints",
    audioSampleMap: aacState2.audioSamples.getSamples(),
    lastSampleObserved: samplesObserved.getLastSampleObserved()
  };
};
var setSeekingHintsForAac = () => {
};
var getSeekingHintsForFlac = ({ flacState: flacState2, samplesObserved }) => {
  return {
    type: "flac-seeking-hints",
    audioSampleMap: flacState2.audioSamples.getSamples(),
    blockingBitStrategy: flacState2.getBlockingBitStrategy() ?? null,
    lastSampleObserved: samplesObserved.getLastSampleObserved()
  };
};
var setSeekingHintsForFlac = ({ hints, state }) => {
...
var getSeekingHintsFromMp4 = ({ structureState: structureState2, isoState, mp4HeaderSegment, mediaSectionState: mediaSectionState2 }) => {
  const structure = structureState2.getIsoStructure();
...
var getSeekingHintsForMp3 = ({ mp3State, samplesObserved, mediaSectionState: mediaSectionState2, contentLength }) => {
  return {
...
var setSeekingHintsForMp3 = ({ hints, state }) => {
...
var getSeekingHintsForRiff = ({ structureState: structureState2, riffState, mediaSectionState: mediaSectionState2 }) => {
  const structure = structureState2.getRiffStructure();
  const strl = getStrlBoxes(structure);
...
var setSeekingHintsForRiff = ({ hints, state }) => {
...
var setSeekingHintsForTransportStream = ({ hints, state }) => {
...
var getSeekingHintsFromWav = ({ structure, mediaSectionState: mediaSectionState2 }) => {
...
var setSeekingHintsForWav = ({ hints, state }) => {
  state.mediaSection.addMediaSection(hints.mediaSection);
};
var getSeekingHintsFromMatroska = (tracksState, keyframesState2, webmState2) => {
  const tracks2 = tracksState.getTracks();
  const firstVideoTrack = tracks2.find((track) => track.type === "video");
  const keyframes = keyframesState2.getKeyframes();
  const loadedCues = webmState2.cues.getIfAlreadyLoaded();
  return {
    ...
    keyframes,
    loadedCues,
    timestampMap: webmState2.getTimeStampMapForSeekingHints()
  };
};
var setSeekingHintsForWebm = ({ hints, state }) => {
...
var getSeekingHints = ({ structureState: structureState2, m3uPlaylistContext, mediaSectionState: mediaSectionState2, isoState, transportStream, tracksState, keyframesState: keyframesState2, webmState: webmState2, flacState: flacState2, samplesObserved, riffState, mp3State, contentLength, aacState: aacState2 }) => {
  const structure = structureState2.getStructureOrNull();
  if (!structure) {
    return null;
  }
  if (structure.type === "iso-base-media") {
  ...
  if (structure.type === "matroska") {
    return getSeekingHintsFromMatroska(tracksState, keyframesState2, webmState2);
  }
  if (structure.type === "transport-stream") {
...
var seekBackwards = async ({ iterator, seekTo, readerInterface, src, controller, logLevel, currentReader: currentReader2, prefetchCache }) => {
  ...
  await currentReader2.getCurrent().abort();
  ...
  currentReader2.setCurrent(newReader);
};
...
var needsToIterateOverSamples = ({ fields, emittedFields }) => {
...
var needsToIterateOverEverySample = ({ fields, emittedFields }) => {
  const keys = Object.keys(fields ?? {});
  const selectedKeys = keys.filter((k) => fields[k]);
  return selectedKeys.some((k) => fieldsNeedEverySampleMap[k] && !emittedFields[k]);
};
var disallowForwardSeekIfSamplesAreNeeded = ({ seekTo, previousPosition, fields }) => {
...
var seekForward = async ({ seekTo, userInitiated, iterator, fields, logLevel, currentReader: currentReader2, readerInterface, src, controller, discardReadBytes, prefetchCache }) => {
  if (userInitiated) {
...
var performSeek = async ({ seekTo, userInitiated, controller, mediaSection, iterator, seekInfiniteLoop, logLevel, mode, contentLength, currentReader: currentReader2, readerInterface, src, discardReadBytes, fields, prefetchCache, isoState }) => {
...
var turnSeekIntoByte = async ({ seek: seek2, mediaSectionState: mediaSectionState2, logLevel, iterator, structureState: structureState2, m3uPlaylistContext, isoState, transportStream, tracksState, webmState: webmState2, keyframes, flacState: flacState2, samplesObserved, riffState, mp3State, contentLength, aacState: aacState2, m3uState: m3uState2, avcState: avcState2 }) => {
...
var workOnSeekRequest = async (options) => {
  const { logLevel, controller, mediaSection, m3uPlaylistContext, isoState, iterator, structureState: structureState2, src, contentLength, readerInterface, mode, seekInfiniteLoop, currentReader: currentReader2, discardReadBytes, fields, transportStream, tracksState, webmState: webmState2, keyframes, flacState: flacState2, samplesObserved, riffState, mp3State, aacState: aacState2, prefetchCache, m3uState: m3uState2, avcState: avcState2 } = options;
...
var emitAvailableInfo = async ({ hasInfo, state }) => {
...
var maySkipOverSamplesInTheMiddle = ({ state }) => {
  const hasCallbacks = getHasCallbacks(state);
...
var parseFrameHeader = ({ iterator, state }) => {
...
var emitSample = async ({ state, data, offset }) => {
  const iterator = getArrayBufferIterator({
    initialData: data,
    maxBytes: data.length,
    logLevel: "error"
  });
  const parsed = parseFrameHeader({
    iterator,
    state
  });
  if (!parsed) {
...
var parseFlacFrame = async ({ state, iterator }) => {
  const blockingBit = state.flac.getBlockingBitStrategy();
  const offset = iterator.counter.getOffset();
  const { returnToCheckpoint } = iterator.startCheckpoint();
  iterator.startReadingBits();
  if (blockingBit === void 0) {
  ...
  while (true) {
    ...
    if (nextByte === 255) {
      ...
      const nextIsLegit = parseFrameHeader({
        iterator,
        state
      });
      if (!nextIsLegit) {
...
var parseFlacHeader = ({ state }) => {
  state.structure.getFlacStructure().boxes.push({
    type: "flac-header"
  });
  return Promise.resolve(null);
};
var parseVorbisComment = ({ state, iterator, size }) => {
...
var parseStreamInfo = async ({ iterator, state }) => {
...
var parseFlacUnkownBlock = ({ iterator, state, size }) => {
  iterator.discard(size);
  state.structure.getFlacStructure().boxes.push({
    type: "flac-header"
  });
...
var parseMetaBlock = ({ iterator, state }) => {
  iterator.startReadingBits();
  ...
  return parseFlacUnkownBlock({
    iterator,
    state,
    size
  });
};
var parseFlac = ({ iterator, state }) => {
...
var calculateSamplePositions = ({ state, mediaSectionStart, trackIds }) => {
  const tracks2 = getTracks(state, true);
  ...
  const moofComplete = areSamplesComplete({
    moofBoxes,
    tfraBoxes
  });
...
var updateSampleIndicesAfterSeek = ({ samplePositionsForMdatStart, seekedByte }) => {
...
var needsTracksForField = ({ field, structure }) => {
  if (field === "dimensions") {
    if (structure?.type === "riff") {
      return false;
    }
...
var makeCanSkipTracksState = ({ hasAudioTrackHandlers, fields, hasVideoTrackHandlers, structure }) => {
...
var getMoovAtom = async ({ endOfMdat, state }) => {
...
var postprocessBytes = ({ bytes, bigEndian, chunkSize }) => {
  if (!bigEndian) {
    return bytes;
  }
  if (chunkSize === null) {
...
var parseMdatSection = async (state) => {
  ...
  if (maySkipOverSamplesInTheMiddle({
    state
  })) {
    const mfra = state.iso.mfra.getIfAlreadyLoaded();
    if (mfra) {
  ...
  const { timestamp: rawCts, decodingTimestamp: rawDts, duration: duration2, isKeyframe, offset, bigEndian, chunkSize } = nextSample.samplePosition;
...
var fetchM3u8Stream = async ({ url, readerInterface }) => {
...
var selectAssociatedPlaylists = async ({ playlists, fn, skipAudioTracks }) => {
...
var selectStream = async ({ streams, fn }) => {
  if (streams.length < 1) {
...
var afterManifestFetch = async ({ structure, m3uState: m3uState2, src, selectM3uStreamFn, logLevel, selectAssociatedPlaylistsFn, readerInterface, onAudioTrack, canSkipTracks }) => {
  ...
  const selectedPlaylist = await selectStream({
    streams,
  ...
  const struc = await Promise.all(playlistUrls.map(async (url) => {
    ...
    const boxes = await fetchM3u8Stream({
      url,
      readerInterface
    });
    return {
...
var parseM3uManifest = ({ iterator, structure, contentLength }) => {
...
var forwardMediaParserControllerPauseResume = ({ parentController, childController }) => {
...
var getLengthAndReader = async ({ canLiveWithoutContentLength, res, ownController, requestedWithoutRange }) => {
...
var validateContentRangeAndDetectIfSupported = ({ requestedRange, parsedContentRange, statusCode }) => {
  if (statusCode === 206) {
    return {
...
var makeFetchRequest = async ({ range: range2, src, controller }) => {
  ...
  const cache = typeof navigator !== "undefined" && navigator.userAgent.includes("Cloudflare-Workers") ? void 0 : "no-store";
...
var cacheKey = ({ src, range: range2 }) => {
  return `${src}-${JSON.stringify(range2)}`;
};
var makeFetchRequestOrGetCached = ({ range: range2, src, controller, logLevel, prefetchCache }) => {
  const key = cacheKey({
    src,
    range: range2
  });
...
var fetchReadContent = async ({ src, range: range2, controller, logLevel, prefetchCache }) => {
...
var fetchPreload = ({ src, range: range2, logLevel, prefetchCache }) => {
  if (typeof src !== "string" && src instanceof URL === false) {
    throw new Error("src must be a string when using `fetchReader`");
  }
  const key = cacheKey({
...
var parseMedia = (options) => {
  ...
  return internalParseMedia({
    ...
    controller: options.controller ?? void 0,
...
var considerSeekBasedOnChunk = async ({ sample, parentController, childController, callback, m3uState: m3uState2, playlistUrl, subtractChunks, chunkIndex }) => {
  const pendingSeek = m3uState2.getSeekToSecondsToProcess(playlistUrl);
  if (pendingSeek === null) {
  ...
  if (timestamp > pendingSeek.targetTime && chunkIndex !== null && chunkIndex > 0) {
    m3uState2.setNextSeekShouldSubtractChunks(playlistUrl, subtractChunks + 1);
    parentController.seek(pendingSeek.targetTime);
    return;
  }
  childController.seek(pendingSeek.targetTime);
  m3uState2.setNextSeekShouldSubtractChunks(playlistUrl, 0);
  m3uState2.setSeekToSecondsToProcess(playlistUrl, null);
};
...
var getChunkToSeekTo = ({ chunks, seekToSecondsToProcess }) => {
  let duration2 = 0;
  for (let i = 0; i < chunks.length; i++) {
...
var processM3uChunk = ({ playlistUrl, state, structure, audioDone, videoDone }) => {
...
var runOverM3u = async ({ state, structure, playlistUrl, logLevel }) => {
...
var getAudioSampleFromCbr = ({ bitrateInKbit, initialOffset, layer, sampleRate, samplesPerFrame, data, state }) => {
...
var getAudioSampleFromVbr = ({ info, position, mp3Info, data }) => {
  if (!mp3Info) {
    throw new Error("No MP3 info");
  }
  const samplesPerFrame = getSamplesPerMpegFrame({
...
var parseMpegHeader = async ({ state }) => {
  const { iterator } = state;
  const initialOffset = iterator.counter.getOffset();
  if (iterator.bytesRemaining() < 32) {
    return;
  }
  const { frameLength, bitrateInKbit, layer, mpegVersion, numberOfChannels, sampleRate, samplesPerFrame } = parseMp3PacketHeader(iterator);
...
var discardUntilSyncword = ({ iterator }) => {
  while (true) {
...
var getKeyFrameOffsetAndPocs = ({ state, sample, trackId }) => {
  if (sample.type === "key") {
...
var convertQueuedSampleToMediaParserSample = ({ sample, state, trackId }) => {
...
var parseListBox = async ({ size, iterator, stateIfExpectingSideEffects }) => {
  const counter = iterator.counter.getOffset();
  const listType = iterator.getByteString(4, false);
  if (listType === "movi") {
...
var parseStrfAudio = ({ iterator, size }) => {
  const box = iterator.startBox(size);
...
var parseStrfVideo = ({ iterator, size }) => {
  const box = iterator.startBox(size);
...
var parseStrf = ({ iterator, size, fccType }) => {
  if (fccType === "vids") {
...
var parseRiffBox = ({ size, id, iterator, stateIfExpectingSideEffects }) => {
  if (id === "LIST") {
    return parseListBox({
      size,
      iterator,
  ...
  if (id === "ISFT") {
    return Promise.resolve(parseIsft({
      iterator,
      size
    }));
  }
  if (id === "avih") {
    return Promise.resolve(parseAvih({
      iterator,
      size
    }));
  }
  if (id === "strh") {
    return Promise.resolve(parseStrh({
      iterator,
      size
    }));
  }
  if (id === "idx1") {
    return Promise.resolve(parseIdx1({
      iterator,
      size
    }));
  }
...
var expectRiffBox = async ({ iterator, stateIfExpectingSideEffects }) => {
...
var handleChunk = async ({ state, ckId, ckSize }) => {
  const { iterator } = state;
...
var parsePmtTable = ({ iterator, tableId, sectionLength }) => {
  const start = iterator.counter.getOffset();
  iterator.getUint16();
  iterator.startReadingBits();
  iterator.getBits(7);
...
var handleAacPacket = async ({ streamBuffer, programId, offset, sampleCallbacks, logLevel, onAudioTrack, transportStream, makeSamplesStartAtZero }) => {
...
var makeTransportStreamPacketBuffer = ({ buffers, pesHeader, offset }) => {
...
var processStreamBuffer = async ({ streamBuffer, programId, structure, sampleCallbacks, logLevel, onAudioTrack, onVideoTrack, transportStream, makeSamplesStartAtZero, avcState: avcState2 }) => {
...
var processFinalStreamBuffers = async ({ structure, sampleCallbacks, logLevel, onAudioTrack, onVideoTrack, transportStream, makeSamplesStartAtZero, avcState: avcState2 }) => {
...
var parseStream = ({ transportStreamEntry, programId, iterator, transportStream }) => {
...
var parsePacket = ({ iterator, structure, transportStream }) => {
  const offset = iterator.counter.getOffset();
...
var canProcessAudio = ({ streamBuffer }) => {
...
var processAudio = async ({ transportStreamEntry, structure, offset, sampleCallbacks, logLevel, onAudioTrack, onVideoTrack, transportStream, makeSamplesStartAtZero, avcState: avcState2 }) => {
  const { streamBuffers, nextPesHeaderStore: nextPesHeader } = transportStream;
  const streamBuffer = streamBuffers.get(transportStreamEntry.pid);
  if (!streamBuffer) {
...
var canProcessVideo = ({ streamBuffer }) => {
...
var processVideo = async ({ programId, structure, streamBuffer, sampleCallbacks, logLevel, onAudioTrack, onVideoTrack, transportStream, makeSamplesStartAtZero, avcState: avcState2 }) => {
  const indexOfSeparator = streamBuffer.get2ndSubArrayIndex();
  if (indexOfSeparator === -1 || indexOfSeparator === 0) {
...
var parseMediaSection2 = async ({ state }) => {
  const { iterator } = state;
  const structure = state.structure.getWavStructure();
...
var getByteForSeek = ({ seekHeadSegment, offset }) => {
...
var expectSegment = async ({ statesForProcessing, isInsideSegment, iterator, logLevel, mediaSectionState: mediaSectionState2 }) => {
  if (iterator.bytesRemaining() === 0) {
...
var parseSegment = async ({ segmentId, length, iterator, headerReadSoFar, statesForProcessing, logLevel }) => {
  if (length < 0) {
...
var selectStatesForProcessing = ({ callbacks, logLevel, onAudioTrack, onVideoTrack, structure, webm, avc }) => {
  return {
    webmState: webm,
    callbacks,
    logLevel,
...
var runParseIteration = async ({ state }) => {
  const structure = state.structure.getStructureOrNull();
...
var parseLoop = async ({ state, throttledState, onError }) => {
...
var warnIfRemotionLicenseNotAcknowledged = ({ acknowledgeRemotionLicense, logLevel, apiName }) => {
...
var setSeekingHints = ({ hints, state }) => {
  if (hints.type === "iso-base-media-seeking-hints") {
    setSeekingHintsForMp4({
      hints,
      state
    });
    return;
  ...
  if (hints.type === "transport-stream-seeking-hints") {
    setSeekingHintsForTransportStream({
      hints,
      state
    });
    return;
...
var getFieldsFromCallback = ({ fields, callbacks }) => {
  const newFields = {
...
var getMfraAtom = async ({ src, contentLength, readerInterface, controller, parentSize, logLevel, prefetchCache }) => {
...
var getMfroAtom = async ({ src, contentLength, readerInterface, controller, logLevel, prefetchCache }) => {
  const result = await readerInterface.read({
    controller,
    range: [
      contentLength - 16,
...
var getMfraSeekingBox = async ({ contentLength, controller, readerInterface, src, logLevel, prefetchCache }) => {
...
var lazyMfraLoad = ({ contentLength, controller, readerInterface, src, logLevel, prefetchCache }) => {
  let prom = null;
  let result = null;
  const triggerLoad = () => {
    if (prom) {
...
var movieTimeScaleState = () => {
  ...
  return {
    ...
    setTrackTimescale: (timescale2) => {
      trackTimescale = timescale2;
    }
  };
};
var isoBaseMediaState = ({ contentLength, controller, readerInterface, src, logLevel, prefetchCache }) => {
  return {
...
var sampleSorter = ({ logLevel, getAllChunksProcessedForPlaylist }) => {
...
var m3uState = (logLevel) => {
  ...
  return {
    ...
    hasEmittedDoneWithTracks: (src) => hasEmittedDoneWithTracks[src] !== void 0,
...
var fetchWebmCues = async ({ src, readerInterface, controller, position, logLevel, prefetchCache }) => {
  const result = await readerInterface.read({
    controller,
    range: position,
    src,
...
var lazyCuesFetch = ({ controller, logLevel, readerInterface, src, prefetchCache }) => {
  let prom = null;
  let sOffset = null;
  let result = null;
...
var webmState = ({ controller, logLevel, readerInterface, src, prefetchCache }) => {
  const trackEntries = {};
  ...
  let timestampMap = /* @__PURE__ */ new Map();
...
var fetchIdx1 = async ({ src, readerInterface, controller, position, logLevel, prefetchCache, contentLength }) => {
...
var lazyIdx1Fetch = ({ controller, logLevel, readerInterface, src, prefetchCache, contentLength }) => {
  let prom = null;
  let result = null;
...
var queuedBFramesState = () => {
  ...
  return {
    addFrame: ({ frame, maxFramesInBuffer, trackId, timescale: timescale2 }) => {
      if (frame.type === "key") {
        flush();
        releasedFrames.push({
          sample: frame,
...
var riffSpecificState = ({ controller, logLevel, readerInterface, src, prefetchCache, contentLength }) => {
  let avcProfile = null;
...
var callbacksState = ({ controller, hasAudioTrackHandlers, hasVideoTrackHandlers, fields, keyframes, emittedFields, samplesObserved, structure, src, seekSignal, logLevel }) => {
...
var samplesObservedState = () => {
  ...
  const videoSamples = /* @__PURE__ */ new Map();
  const audioSamples = /* @__PURE__ */ new Map();
  ...
  const addVideoSample = (videoSample) => {
    ...
    if (largestVideoSample === void 0 || presentationTimeInSeconds > largestVideoSample) {
      largestVideoSample = presentationTimeInSeconds + duration2;
    }
    if (smallestVideoSample === void 0 || presentationTimeInSeconds < smallestVideoSample) {
  ...
  const addAudioSample = (audioSample) => {
    ...
    if (largestAudioSample === void 0 || presentationTimeInSeconds > largestAudioSample) {
      largestAudioSample = presentationTimeInSeconds + duration2;
    }
    if (smallestAudioSample === void 0 || presentationTimeInSeconds < smallestAudioSample) {
...
var seekInfiniteLoopDetectionState = () => {
  ...
  return {
    registerSeek: (byte) => {
      ...
      if (lastSeek.numberOfTimes >= 10 && firstSeekTime && now - firstSeekTime <= 2e3) {
...
var transportStreamState = () => {
  const streamBuffers = /* @__PURE__ */ new Map();
...
var throttledStateUpdate = ({ updateFn, everyMilliseconds, controller }) => {
  let currentState = {
    bytes: 0,
    percentage: null,
    totalBytes: null
...
var internalParseMedia = async function({ src, fields: _fieldsInReturnValue, reader: readerInterface, onAudioTrack, onVideoTrack, controller = mediaParserController(), logLevel, onParseProgress: onParseProgressDoNotCallDirectly, progressIntervalInMs, mode, onDiscardedData, onError, acknowledgeRemotionLicense, apiName, selectM3uStream: selectM3uStreamFn, selectM3uAssociatedPlaylists: selectM3uAssociatedPlaylistsFn, m3uPlaylistContext, makeSamplesStartAtZero, seekingHints, ...more }) {
  ...
  controller._internals.attachSimulateSeekResolution((seek2) => {
    const { aacState: aacState2, avcState: avcState2, flacState: flacState2, isoState, iterator, keyframes, m3uState: m3uState2, mediaSection, mp3State, riffState, samplesObserved, structureState: structureState2, tracksState, transportStream, webmState: webmState2 } = getWorkOnSeekRequestOptions(state);
...
// node_modules/.deno/@remotion+media-parser@4.0.421/node_modules/@remotion/media-parser/dist/esm/web.mjs
var MediaParserAbortError2 = class extends Error {
  constructor(message) {
    super(message);
    this.name = "MediaParserAbortError";
    this.cause = void 0;
...
var getNumberForLogLevel2 = (level) => {
  return logLevels2.indexOf(level);
};
var isEqualOrBelowLogLevel2 = (currentLevel, level) => {
  return getNumberForLogLevel2(currentLevel) <= getNumberForLogLevel2(level);
};
var Log2 = {
  trace: (logLevel, ...args) => {
    if (isEqualOrBelowLogLevel2(logLevel, "trace")) {
      return console.log(...args);
    }
  },
  verbose: (logLevel, ...args) => {
    if (isEqualOrBelowLogLevel2(logLevel, "verbose")) {
      return console.log(...args);
    }
  },
  info: (logLevel, ...args) => {
    if (isEqualOrBelowLogLevel2(logLevel, "info")) {
      return console.log(...args);
    }
  },
  warn: (logLevel, ...args) => {
    if (isEqualOrBelowLogLevel2(logLevel, "warn")) {
      return console.warn(...args);
    }
  },
  error: (...args) => {
...
var getLengthAndReader2 = async ({ canLiveWithoutContentLength, res, ownController, requestedWithoutRange }) => {
  const length = res.headers.get("content-length");
  const contentLength = length === null ? null : parseInt(length, 10);
  if (requestedWithoutRange || canLiveWithoutContentLength && contentLength === null) {
    const buffer = await res.arrayBuffer();
...
function parseContentRange2(input) {
  const matches = input.match(/^(\w+) ((\d+)-(\d+)|\*)\/(\d+|\*)$/);
  if (!matches) return null;
  const [, unit, , start, end, size] = matches;
  const range2 = {
...
var validateContentRangeAndDetectIfSupported2 = ({ requestedRange, parsedContentRange, statusCode }) => {
  if (statusCode === 206) {
    return {
      supportsContentRange: true
    };
...
var makeFetchRequest2 = async ({ range: range2, src, controller }) => {
  const resolvedUrl = resolveUrl2(src);
  const resolvedUrlString = resolvedUrl.toString();
  if (!resolvedUrlString.startsWith("https://") && !resolvedUrlString.startsWith("blob:") && !resolvedUrlString.startsWith("data:") && !resolvedUrlString.startsWith("http://")) {
    return Promise.reject(new Error(`${resolvedUrlString} is not a URL - needs to start with http:// or https:// or blob:. If you want to read a local file, pass \`reader: nodeReader\` to parseMedia().`));
  }
  ...
  const contentRange = res.headers.get("content-range");
  const parsedContentRange = contentRange ? parseContentRange2(contentRange) : null;
  if (!res.ok) {
    throw new Error(`Server returned status code ${res.status} for ${resolvedUrl} and range ${requestedRange}`);
  }
  const { supportsContentRange } = validateContentRangeAndDetectIfSupported2({
    requestedRange,
    parsedContentRange,
    statusCode: res.status
  });
  if (controller) {
    controller._internals.signal.addEventListener("abort", () => {
      ownController.abort(new MediaParserAbortError2("Aborted by user"));
    }, {
      once: true
    });
  }
  const contentDisposition = res.headers.get("content-disposition");
  const name = contentDisposition?.match(/filename="([^"]+)"/)?.[1];
  const { contentLength, needsContentRange, reader } = await getLengthAndReader2({
    canLiveWithoutContentLength,
    res,
    ownController,
    requestedWithoutRange: requestWithoutRange
...
var cacheKey2 = ({ src, range: range2 }) => {
  return `${src}-${JSON.stringify(range2)}`;
};
var makeFetchRequestOrGetCached2 = ({ range: range2, src, controller, logLevel, prefetchCache }) => {
  const key = cacheKey2({
    src,
    range: range2
  });
  const cached = prefetchCache.get(key);
  if (cached) {
    Log2.verbose(logLevel, `Reading from preload cache for ${key}`);
    return cached;
  }
  Log2.verbose(logLevel, `Fetching ${key}`);
  const result = makeFetchRequest2({
    range: range2,
    src,
    controller
  });
...
var fetchReadContent2 = async ({ src, range: range2, controller, logLevel, prefetchCache }) => {
  if (typeof src !== "string" && src instanceof URL === false) {
    throw new Error("src must be a string when using `fetchReader`");
  }
  const fallbackName = src.toString().split("/").pop();
  const res = makeFetchRequestOrGetCached2({
    range: range2,
    src,
    controller,
    logLevel,
...
var fetchPreload2 = ({ src, range: range2, logLevel, prefetchCache }) => {
  if (typeof src !== "string" && src instanceof URL === false) {
    throw new Error("src must be a string when using `fetchReader`");
  }
  const key = cacheKey2({
...
var fetchReadWholeAsText2 = async (src) => {
  if (typeof src !== "string" && src instanceof URL === false) {
    throw new Error("src must be a string when using `fetchReader`");
  }
  const res = await fetch(src);
...
var fetchCreateAdjacentFileSource2 = (relativePath, src) => {
  if (typeof src !== "string" && src instanceof URL === false) {
    throw new Error("src must be a string or URL when using `fetchReader`");
  }
  return new URL(relativePath, src).toString();
};
var webFileReadContent2 = ({ src, range: range2, controller }) => {
  if (typeof src === "string" || src instanceof URL) {
    throw new Error("`inputTypeFileReader` only supports `File` objects");
  }
  const part = range2 === null ? src : typeof range2 === "number" ? src.slice(range2) : src.slice(range2[0], range2[1] + 1);
...
var webFileReadWholeAsText2 = () => {
  throw new Error("`webFileReader` cannot read auxiliary files.");
};
var webFileCreateAdjacentFileSource2 = () => {
  throw new Error("`webFileReader` cannot create adjacent file sources.");
};
var webReader2 = {
  read: (params) => {
    if (params.src instanceof Blob) {
      return webFileReadContent2(params);
    }
    return fetchReadContent2(params);
  },
  createAdjacentFileSource: (relativePath, src) => {
    if (src instanceof Blob) {
      return webFileCreateAdjacentFileSource2(relativePath, src);
    }
    return fetchCreateAdjacentFileSource2(relativePath, src);
  },
  readWholeAsText: (src) => {
    if (src instanceof Blob) {
      return webFileReadWholeAsText2(src);
    }
    return fetchReadWholeAsText2(src);
  },
  preload: ({ range: range2, src, logLevel, prefetchCache }) => {
    if (src instanceof Blob) {
      return;
    }
    return fetchPreload2({
      range: range2,
      src,
      logLevel,
      prefetchCache
...
// node_modules/.deno/@remotion+webcodecs@4.0.421/node_modules/@remotion/webcodecs/dist/esm/index.mjs
...
var canUseWebFsWriter = async () => {
  ...
  try {
    ...
    const canUse = fileHandle.createWritable !== void 0;
    return canUse;
  } catch {
    return false;
  }
...
var ensureMultipleOfTwo = ({ dimensions, needsToBeMultipleOfTwo }) => {
...
var calculateNewSizeAfterResizing = ({ dimensions, resizeOperation, needsToBeMultipleOfTwo }) => {
...
var calculateNewDimensionsFromRotate = ({ height, width, rotation }) => {
  const normalized = normalizeVideoRotation2(rotation);
...
var calculateNewDimensionsFromRotateAndScale = ({ width, height, rotation, resizeOperation, needsToBeMultipleOfTwo }) => {
...
var normalizeVideoRotation2 = (rotation) => {
  return (rotation % 360 + 360) % 360;
};
var rotateAndResizeVideoFrame = ({ frame, rotation, needsToBeMultipleOfTwo = false, resizeOperation }) => {
  const normalized = normalizeVideoRotation2(rotation);
  ...
  const canvasRotationToApply = normalizeVideoRotation2(normalized);
...
var withResolversAndWaitForReturn = () => {
  const { promise, reject, resolve } = withResolvers2();
  const { promise: returnPromise, resolve: resolveReturn } = withResolvers2();
  return {
    getPromiseToImmediatelyReturn: () => {
      resolveReturn(void 0);
      return promise;
    },
    reject: (reason) => {
...
var { Log: Log3 } = MediaParserInternals;
var makeTimeoutPromise = ({ label, ms, controller }) => {
  const { promise, reject, resolve } = withResolvers2();
  let timeout = null;
  const set = () => {
    timeout = setTimeout(() => {
...
var makeIoSynchronizer = ({ logLevel, label, controller }) => {
  ...
  const printState = (prefix) => {
    Log3.trace(logLevel, `[${label}] ${prefix}, state: Last input = ${lastInput} Last output = ${lastOutput} Inputs since last output = ${inputsSinceLastOutput}, Queue = ${getQueuedItems()}`);
  };
  ...
  const waitForOutput = () => {
    const { promise, resolve } = withResolvers2();
    const on = () => {
...
var convertAudioData = ({ audioData, newSampleRate = audioData.sampleRate, format = audioData.format }) => {
  ...
  if (newSampleRate < 3e3 || newSampleRate > 768e3) {
...
var getWaveAudioEncoder = ({ onChunk, controller, config, ioSynchronizer }) => {
  return {
    close: () => {
      return Promise.resolve();
    },
    encode: (unconvertedAudioData) => {
      ...
      const chunk = {
        ...
        type: "key",
        copyTo: (destination) => audioData.copyTo(destination, {
          planeIndex: 0
        }),
        byteLength: audioData.allocationSize({
          planeIndex: 0
        })
      };
...
var createAudioEncoder = ({ onChunk, onError, codec, controller, config: audioEncoderConfig, logLevel, onNewAudioSampleRate }) => {
  if (controller._internals._mediaParserController._internals.signal.aborted) {
...
var isSameVideoCodec = ({ inputVideoCodec, outputCodec }) => {
  if (outputCodec === "h264") {
...
var isSameAudioCodec = ({ inputAudioCodec, outputCodec }) => {
  if (outputCodec === "aac") {
...
var canCopyAudioTrack = ({ inputCodec, outputContainer, inputContainer, outputAudioCodec }) => {
  if (outputAudioCodec) {
    if (!isSameAudioCodec({
...
var canCopyVideoTrack = ({ outputContainer, rotationToApply, inputContainer, resizeOperation, inputTrack, outputVideoCodec }) => {
  if (normalizeVideoRotation2(inputTrack.rotation) !== normalizeVideoRotation2(rotationToApply)) {
    return false;
  }
  if (outputVideoCodec) {
    if (!isSameVideoCodec({
...
var getAudioEncoderConfig = async (config) => {
  ...
  const maybeItIsTheSampleRateThatIsTheProblem = config.sampleRate !== 48e3 && config.sampleRate !== 44100;
  if (maybeItIsTheSampleRateThatIsTheProblem) {
    return getAudioEncoderConfig({
      ...config,
      sampleRate: config.sampleRate === 22050 ? 44100 : 48e3
    });
  }
  return null;
};
var canReencodeAudioTrack = async ({ track, audioCodec, bitrate, sampleRate }) => {
...
var chooseCorrectAvc1Profile = ({ width, height, fps }) => {
  const profiles = [
    {
      level: "3.1",
      hex: "1F",
...
var hevcLevels = [
  ...
  {
    level: "4",
    maxBitrateMainTier: 12e3,
    maxBitrateHighTier: 3e4,
    maxResolutionsAndFrameRates: [
      {
        width: 1280,
        height: 720,
  ...
  {
    level: "4.1",
    maxBitrateMainTier: 2e4,
    maxBitrateHighTier: 5e4,
    maxResolutionsAndFrameRates: [
      {
        width: 1280,
        height: 720,
  ...
  {
    level: "5.1",
    maxBitrateMainTier: 4e4,
    maxBitrateHighTier: 16e4,
    maxResolutionsAndFrameRates: [
      {
        width: 1920,
        height: 1080,
  ...
  {
    level: "5.2",
    maxBitrateMainTier: 6e4,
    maxBitrateHighTier: 24e4,
    maxResolutionsAndFrameRates: [
      {
        width: 2048,
        height: 1080,
  ...
  {
    level: "6.1",
    maxBitrateMainTier: 12e4,
    maxBitrateHighTier: 48e4,
    maxResolutionsAndFrameRates: [
      {
        width: 3840,
        height: 2160,
  ...
  {
    level: "6.2",
    maxBitrateMainTier: 24e4,
    maxBitrateHighTier: 8e5,
    maxResolutionsAndFrameRates: [
      {
        width: 3840,
        height: 2160,
...
var chooseCorrectHevcProfile = ({ width, height, fps }) => {
...
var getCodecStringForEncoder = ({ codec, fps, height, width }) => {
  if (codec === "h264") {
    return chooseCorrectAvc1Profile({
      fps,
      height,
      width
    });
  ...
  if (codec === "h265") {
    return chooseCorrectHevcProfile({
      fps,
      height,
      width
    });
...
var getVideoEncoderConfig = async ({ codec, height, width, fps }) => {
  if (typeof VideoEncoder === "undefined") {
    return null;
  }
  const config = {
    codec: getCodecStringForEncoder({
      codec,
      fps,
      height,
      width
    ...
    height,
    width,
    bitrate: isSafari() ? 3e6 : void 0,
    bitrateMode: codec === "vp9" && !isSafari() ? "quantizer" : void 0,
    framerate: fps ?? void 0
  };
  const hardware = {
    ...config,
    hardwareAcceleration: "prefer-hardware"
...
var canReencodeVideoTrack = async ({ videoCodec, track, resizeOperation, rotate }) => {
...
var autoSelectWriter = async (writer, logLevel) => {
  if (writer) {
    Log3.verbose(logLevel, "Using writer provided by user");
    return writer;
  }
  Log3.verbose(logLevel, "Determining best writer");
  const hasNavigator = typeof navigator !== "undefined";
  if (!hasNavigator) {
    Log3.verbose(logLevel, "No navigator API detected, using buffer writer");
    return bufferWriter;
  }
  const isOffline = !navigator.onLine;
  if (isOffline) {
    Log3.verbose(logLevel, "Offline mode detected, using buffer writer");
    return bufferWriter;
  }
  try {
    const { promise: timeout, reject, resolve } = withResolvers2();
    const time = setTimeout(() => reject(new Error("WebFS check timeout")), 2e3);
    ...
    clearTimeout(time);
    if (webFsSupported) {
      Log3.verbose(logLevel, "Using WebFS writer because it is supported");
      return webFsWriter;
    }
  } catch (err) {
    Log3.verbose(logLevel, `WebFS check failed: ${err}. Falling back to buffer writer`);
  }
  Log3.verbose(logLevel, "Using buffer writer because WebFS writer is not supported or unavailable");
  return bufferWriter;
};
var calculateProgress = ({ millisecondsWritten, expectedOutputDurationInMs }) => {
...
var getIdForName = (name) => {
  const value = Object.entries(MediaParserInternals.matroskaElements).find(([key]) => key === name)?.[1];
  if (!value) {
...
var makeFromStructure = (fields) => {
  ...
  if (struct.type === "children") {
    ...
    return {
      bytes: combineUint8Arrays2(arrays),
      offsets: {
        offset: 0,
        children,
        field: fields.type
...
var combineUint8Arrays2 = (arrays) => {
  if (arrays.length === 0) {
    return new Uint8Array([]);
  }
  if (arrays.length === 1) {
...
var makeMatroskaBytes = (fields) => {
  ...
  const bytes = combineUint8Arrays2([
    header,
    size,
    value.bytes
  ]);
...
var padMatroskaBytes = (fields, totalLength) => {
  ...
  const paddingLength = totalLength - regular.bytes.byteLength - matroskaToHex(MediaParserInternals.matroskaElements.Void).byteLength;
  if (paddingLength < 0) {
...
function serializeUint162(value) {
  const buffer = new ArrayBuffer(2);
  const view = new DataView(buffer);
  view.setUint16(0, value);
  return new Uint8Array(buffer);
...
var addSize = (arr) => {
  return combineUint8Arrays2([
    numberTo32BitUIntOrInt(arr.length + 4),
    arr
  ]);
};
...
var createFtyp = ({ majorBrand, minorBrand, compatibleBrands }) => {
  ...
  const compatibleBrandsArr = combineUint8Arrays2(compatibleBrands.map((b) => stringsToUint8Array(b)));
  return addSize(combineUint8Arrays2([
    type,
    majorBrandArr,
    minorBrandArr,
    compatibleBrandsArr
...
var createIsoBaseMediaFtyp = ({ majorBrand, minorBrand, compatibleBrands }) => {
  return createFtyp({
    compatibleBrands,
    majorBrand,
    minorBrand
...
var createMoov = ({ mvhd, traks, udta }) => {
  return addSize(combineUint8Arrays2([
...
var fromUnixTimestamp = (value) => {
  ...
  const baseDate = /* @__PURE__ */ new Date("1904-01-01T00:00:00Z");
  return Math.floor(value / 1e3 - baseDate.getTime() / 1e3);
};
var createMvhd = ({ timescale: timescale2, durationInUnits, rate, volume, nextTrackId, matrix, creationTime, modificationTime }) => {
  if (matrix.length !== 9) {
    throw new Error("Matrix must be 9 elements long");
  }
  const content = combineUint8Arrays2([
    ...
    numberTo32BitUIntOrInt(timescale2),
    ...
    combineUint8Arrays2(new Array(6).fill(new Uint8Array([
      0,
      0,
      0,
      0
...
var calculateAReasonableMp4HeaderLength = ({ expectedDurationInSeconds, expectedFrameRate }) => {
  ...
  const bytesPerSecond = 3.7 * 1024 * 1024 / 6e3;
...
var createAvc1Data = ({ avccBox, pasp, width, height, horizontalResolution, verticalResolution, compressorName, depth }) => {
  return addSize(combineUint8Arrays2([
...
var createHvc1Data = ({ compressorName, depth, height, horizontalResolution, hvccBox, pasp, verticalResolution, width }) => {
  return addSize(combineUint8Arrays2([
...
var createMp4a = ({ sampleRate, channelCount, avgBitrate, maxBitrate, codecPrivate: codecPrivate2 }) => {
  if (!codecPrivate2) {
    throw new Error("Need codecPrivate for mp4a");
  }
  const esdsAtom = addSize(combineUint8Arrays2([
    ...
    addLeading128Size(combineUint8Arrays2([
      ...
      addLeading128Size(new Uint8Array([
        2
      ]))
    ]))
  ]));
...
var createMdhd = ({ creationTime, modificationTime, timescale: timescale2, duration: duration2 }) => {
  return addSize(combineUint8Arrays2([
    ...
    modificationTime === null ? numberTo32BitUIntOrInt(0) : numberTo32BitUIntOrInt(fromUnixTimestamp(modificationTime)),
    numberTo32BitUIntOrInt(timescale2),
    numberTo32BitUIntOrInt(Math.round(duration2 / 1e3 * timescale2)),
    new Uint8Array([
      85,
      196
    ]),
...
var createTkhdForAudio = ({ creationTime, modificationTime, flags, trackId, duration: duration2, volume, timescale: timescale2 }) => {
  return addSize(combineUint8Arrays2([
...
var createTkhdForVideo = ({ creationTime, modificationTime, duration: duration2, trackId, volume, matrix, width, height, flags, timescale: timescale2 }) => {
  const content = combineUint8Arrays2([
    stringsToUint8Array("tkhd"),
    new Uint8Array([
      0
    ...
    numberTo32BitUIntOrInt(duration2 / 1e3 * timescale2),
    new Uint8Array([
      0,
      0,
      0,
...
var createMinf = ({ vmhdAtom, stblAtom }) => {
  return addSize(combineUint8Arrays2([
...
var createStcoAtom = (samplePositions) => {
  ...
  return addSize(combineUint8Arrays2([
    ...
    combineUint8Arrays2(chunkOffsets.map((offset) => needs64Bit ? numberTo64BitUIntOrInt(offset) : numberTo32BitUIntOrInt(offset)))
  ]));
};
...
var createStss = (samplePositions) => {
  const samples = samplePositions.map((sample, i) => [
    sample.isKeyframe,
    i
  ]).filter((s) => s[0]).map((s) => s[1] + 1);
  return addSize(combineUint8Arrays2([
...
var createSttsAtom = (samplePositions) => {
  ...
  const durations = samplePositions.map((_, i, a) => {
    if (a[i].duration === void 0 || a[i].duration === 0) {
      if (a[i + 1] === void 0) {
...
var createStbl = ({ samplePositions, codecSpecificData, isVideo }) => {
...
var createHdlr = (type) => {
  return addSize(combineUint8Arrays2([
    ...
    type === "mdir" ? numberTo32BitUIntOrInt(1634758764) : new Uint8Array([
      0,
      0,
      0,
      0
...
var serializeTrack = ({ track, durationInUnits, samplePositions, timescale: timescale2 }) => {
...
var createPaddedMoovAtom = ({ durationInUnits, trackInfo, timescale: timescale2, expectedDurationInSeconds, logLevel, expectedFrameRate }) => {
  const headerLength = calculateAReasonableMp4HeaderLength({
    expectedDurationInSeconds,
    expectedFrameRate
  });
  if (expectedDurationInSeconds !== null) {
    Log3.verbose(logLevel, `Expecting duration of the video to be ${expectedDurationInSeconds} seconds, allocating ${headerLength} bytes for the MP4 header.`);
  } else {
    Log3.verbose(logLevel, `No duration was provided, allocating ${headerLength} bytes for the MP4 header.`);
  }
...
var CONTAINER_TIMESCALE = 1e3;
var createIsoBaseMedia = async ({ writer, onBytesProgress, onMillisecondsProgress, logLevel, filename, progressTracker, expectedDurationInSeconds, expectedFrameRate }) => {
  ...
  await w.write(combineUint8Arrays2([
  ...
  const addCodecPrivateToTrack = ({ trackNumber: trackNumber2, codecPrivate: codecPrivate2 }) => {
  ...
  const addSample = async ({ chunk, trackNumber: trackNumber2, isVideo, codecPrivate: codecPrivate2 }) => {
  ...
  return {
    ...
    async waitForFinish() {
      MediaParserInternals.Log.verbose(logLevel, "All write operations queued. Waiting for finish...");
      await Promise.all(waitForFinishPromises.map((p) => p()));
      MediaParserInternals.Log.verbose(logLevel, "Cleanup tasks executed");
      await operationProm.current;
      await updateMoov();
      await updateMdatSize();
      MediaParserInternals.Log.verbose(logLevel, "All write operations done. Waiting for finish...");
      await w.finish();
    }
  };
};
...
var makeSimpleBlock = ({ bytes, trackNumber: trackNumber2, timecodeRelativeToCluster, keyframe, invisible, lacing }) => {
  const simpleBlockHeader = matroskaToHex("0xa3");
  const headerByte = Number(keyframe) << 7 | Number(invisible) << 3 | lacing << 1;
  const body = combineUint8Arrays2([
    getVariableInt(trackNumber2, null),
    serializeUint162(timecodeRelativeToCluster),
    new Uint8Array([
      headerByte
    ]),
    bytes
...
var timestampToClusterTimestamp = (timestamp, timescale2) => {
  return Math.round(timestamp / timescale2 * 1e3);
};
var canFitInCluster = ({ clusterStartTimestamp, chunk, timescale: timescale2 }) => {
  const timecodeRelativeToCluster = timestampToClusterTimestamp(chunk.timestamp, timescale2) - timestampToClusterTimestamp(clusterStartTimestamp, timescale2);
...
var makeCluster = async ({ writer, clusterStartTimestamp, timescale: timescale2, logLevel }) => {
  Log3.verbose(logLevel, `Making new Matroska cluster with timestamp ${clusterStartTimestamp}`);
  const cluster = createClusterSegment(timestampToClusterTimestamp(clusterStartTimestamp, timescale2));
  const clusterVIntPosition = writer.getWrittenByteCount() + cluster.offsets.offset + matroskaToHex(MediaParserInternals.matroskaElements.Cluster).byteLength;
  let clusterSize = cluster.bytes.byteLength - matroskaToHex(MediaParserInternals.matroskaElements.Cluster).byteLength - CLUSTER_MIN_VINT_WIDTH;
  ...
  const shouldMakeNewCluster = ({ isVideo, chunk, newT }) => {
    const newTimestamp = timestampToClusterTimestamp(newT, timescale2);
    const oldTimestamp = timestampToClusterTimestamp(clusterStartTimestamp, timescale2);
    ...
    if (!canFit) {
      Log3.verbose(logLevel, `Cannot fit ${chunk.timestamp} in cluster ${clusterStartTimestamp}. Creating new cluster`);
      return true;
    }
    const keyframe = chunk.type === "key";
    return newTimestamp - oldTimestamp >= 2e3 && keyframe && isVideo;
  };
  return {
    addSample,
...
var getRangeValue = ({ transferCharacteristics: transferCharacteristics2, matrixCoefficients: matrixCoefficients2, fullRange }) => {
  return transferCharacteristics2 && matrixCoefficients2 ? 3 : fullRange === true ? 2 : fullRange === false ? 1 : 0;
};
var getPrimariesValue = (primaries2) => {
  if (primaries2 === null) {
    return null;
  }
  if (primaries2 === "bt709") {
  ...
  if (primaries2 === "bt470bg") {
    return 5;
  }
  if (primaries2 === "smpte170m") {
    return 6;
  }
  if (primaries2 === "bt2020") {
    return 9;
  }
  if (primaries2 === "smpte432") {
    return 12;
  }
  throw new Error("Unknown primaries " + primaries2);
};
var getTransferCharacteristicsValue = (transferCharacteristics2) => {
  if (transferCharacteristics2 === null) {
    return null;
  }
  if (transferCharacteristics2 === "bt709") {
    return 1;
  }
  if (transferCharacteristics2 === "smpte170m") {
    return 6;
  }
  if (transferCharacteristics2 === "iec61966-2-1") {
    return 13;
  }
  if (transferCharacteristics2 === "linear") {
    return 8;
  }
  if (transferCharacteristics2 === "pq") {
    return 16;
  }
  if (transferCharacteristics2 === "hlg") {
    return 18;
  }
  throw new Error("Unknown transfer characteristics " + transferCharacteristics2);
};
var getMatrixCoefficientsValue = (matrixCoefficients2) => {
  if (matrixCoefficients2 === null) {
    return null;
  }
  if (matrixCoefficients2 === "rgb") {
    return 0;
  }
  if (matrixCoefficients2 === "bt709") {
    return 1;
  }
  if (matrixCoefficients2 === "bt470bg") {
    return 5;
  }
  if (matrixCoefficients2 === "smpte170m") {
    return 6;
  }
  if (matrixCoefficients2 === "bt2020-ncl") {
    return 9;
  }
  throw new Error("Unknown matrix coefficients " + matrixCoefficients2);
};
var makeMatroskaColorBytes = ({ transfer: transferCharacteristics2, matrix: matrixCoefficients2, primaries: primaries2, fullRange }) => {
  const rangeValue = getRangeValue({
    transferCharacteristics: transferCharacteristics2,
    matrixCoefficients: matrixCoefficients2,
    fullRange
  });
  const primariesValue = getPrimariesValue(primaries2);
  const transferChracteristicsValue = getTransferCharacteristicsValue(transferCharacteristics2);
  if (matrixCoefficients2 === "rgb") {
    throw new Error("Cannot encode Matroska in RGB");
  }
  const matrixCoefficientsValue = getMatrixCoefficientsValue(matrixCoefficients2);
  return makeMatroskaBytes({
    type: "Colour",
    minVintWidth: null,
    value: [
...
var makeMatroskaVideoBytes = ({ color: color2, width, height }) => {
  return makeMatroskaBytes({
    type: "Video",
    value: [
      {
...
var makeMatroskaAudioTrackEntryBytes = ({ trackNumber: trackNumber2, codec, numberOfChannels, sampleRate, codecPrivate: codecPrivate2 }) => {
  return makeMatroskaBytes({
    type: "TrackEntry",
    minVintWidth: null,
    value: [
...
var makeMatroskaVideoTrackEntryBytes = ({ color: color2, width, height, trackNumber: trackNumber2, codec, codecPrivate: codecPrivate2 }) => {
  return makeMatroskaBytes({
    type: "TrackEntry",
    minVintWidth: null,
    value: [
...
var { matroskaElements: matroskaElements2 } = MediaParserInternals;
var timescale = 1e6;
var createMatroskaMedia = async ({ writer, onBytesProgress, onMillisecondsProgress, filename, logLevel, progressTracker }) => {
  ...
  seeks.push({
    hexString: matroskaElements2.Info,
    byte: infoOffset - seekHeadOffset
  });
  seeks.push({
    hexString: matroskaElements2.Tracks,
    byte: tracksOffset - seekHeadOffset
  });
  const updateSeekWrite = async () => {
    const updatedSeek = createMatroskaSeekHead(seeks);
    await w.updateDataAt(seekHeadOffset, combineUint8Arrays2(updatedSeek.map((b) => b.bytes)));
    onBytesProgress(w.getWrittenByteCount());
  };
  ...
  const updateSegmentSize = async (size) => {
    ...
    await w.updateDataAt(segmentOffset + matroskaToHex(matroskaElements2.Segment).byteLength, data);
    onBytesProgress(w.getWrittenByteCount());
  };
  ...
  seeks.push({
    hexString: matroskaElements2.Cluster,
  ...
  const addSample = async ({ chunk, trackNumber: trackNumber2, isVideo }) => {
    const offset = w.getWrittenByteCount();
    const { cluster, isNew, smallestProgress } = await getClusterOrMakeNew({
      chunk,
      isVideo
    });
    const newDuration = Math.round((chunk.timestamp + (chunk.duration ?? 0)) / 1e3);
  ...
  const addTrack = async (track) => {
    ...
    await w.updateDataAt(tracksOffset, combineUint8Arrays2(newTracks.map((b) => b.bytes)));
  };
  const operationProm = {
    current: Promise.resolve()
  };
  ...
  return {
    ...
    addSample: ({ chunk, trackNumber: trackNumber2, isVideo }) => {
      operationProm.current = operationProm.current.then(() => addSample({
        chunk,
        trackNumber: trackNumber2,
        isVideo
      }));
    ...
    addTrack: (track) => {
      const trackNumber2 = currentTracks.length + 1;
      operationProm.current = operationProm.current.then(() => addTrack({
        ...track,
        trackNumber: trackNumber2
      }));
      trackNumbers.push(trackNumber2);
      return operationProm.current.then(() => ({
        trackNumber: trackNumber2
      }));
    },
    addWaitForFinishPromise: (promise) => {
    ...
    async waitForFinish() {
      ...
      if (cuesBytes) {
        seeks.push({
          hexString: matroskaElements2.Cues,
          byte: w.getWrittenByteCount() - seekHeadOffset
        });
        await w.write(cuesBytes.bytes);
      }
      await updateSeekWrite();
      const segmentSize = w.getWrittenByteCount() - segmentOffset - matroskaToHex(matroskaElements2.Segment).byteLength - MATROSKA_SEGMENT_MIN_VINT_WIDTH;
...
var createWav = async ({ filename, logLevel, onBytesProgress, onMillisecondsProgress, writer, progressTracker }) => {
  const w = await writer.createContent({
    filename,
    mimeType: "audio/wav",
    logLevel
  ...
  const updateChannelNum = async (numberOfChannels) => {
    await w.updateDataAt(channelNumPosition, new Uint8Array([
      numberOfChannels,
      0
    ]));
  };
  ...
  const addSample = async (chunk) => {
    Log3.trace(logLevel, "Adding sample", chunk);
    await w.write(chunk.data);
    onMillisecondsProgress((chunk.timestamp + (chunk.duration ?? 0)) / 1e3);
    onBytesProgress(w.getWrittenByteCount());
  };
  const waitForFinishPromises = [];
  return {
    ...
    async waitForFinish() {
      Log3.verbose(logLevel, "All write operations queued. Waiting for finish...");
      await Promise.all(waitForFinishPromises.map((p) => p()));
      await operationProm.current;
      await updateSize();
      await w.finish();
...
var makeProgressTracker = () => {
  ...
  return {
    ...
    updateTrackProgress: (trackNumber2, progress) => {
      if (trackNumberProgresses[trackNumber2] === void 0) {
...
var availableVideoCodecs = [
  "vp8",
  "vp9",
  "h264",
  "h265"
];
var copyAudioTrack = async ({ state, track, logLevel, onMediaStateUpdate, progressTracker }) => {
  ...
  Log3.verbose(logLevel, `Copying audio track ${track.trackId} as track ${addedTrack.trackNumber}. Timescale = ${track.originalTimescale}, codec = ${track.codecEnum} (${track.codec}) `);
...
var DEFAULT_BITRATE = 128e3;
var defaultOnAudioTrackHandler = async ({ track, defaultAudioCodec, logLevel, canCopyTrack }) => {
  const bitrate = DEFAULT_BITRATE;
  if (canCopyTrack) {
    MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (audio): Can copy track, therefore copying`);
    return Promise.resolve({
      type: "copy"
    });
  }
  if (defaultAudioCodec === null) {
    MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (audio): Container does not support audio, dropping audio`);
    return Promise.resolve({
      type: "drop"
    });
  }
  ...
  if (canReencode) {
    MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (audio): Cannot copy, but re-encode, therefore re-encoding`);
    return Promise.resolve({
      type: "reencode",
      bitrate,
      audioCodec: defaultAudioCodec,
  ...
  MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (audio): Can neither re-encode nor copy, failing render`);
  return Promise.resolve({
    type: "fail"
  });
};
var getDefaultAudioCodec = ({ container }) => {
  if (container === "webm") {
    return "opus";
  }
  if (container === "mp4") {
...
var getWaveAudioDecoder = ({ onFrame, config, sampleFormat, ioSynchronizer, onError }) => {
...
var VideoUndecodableError = class _VideoUndecodableError extends Error {
  config;
  constructor({ message, config }) {
    ...
    this.config = config;
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, _VideoUndecodableError);
    }
  }
};
var AudioUndecodableError = class _AudioUndecodableError extends Error {
  config;
  constructor({ message, config }) {
    super(message);
    ...
    this.config = config;
    if (Error.captureStackTrace) {
      Error.captureStackTrace(this, _AudioUndecodableError);
    }
  }
};
var internalCreateAudioDecoder = async ({ onFrame, onError, controller, config, logLevel }) => {
...
var reencodeAudioTrack = async ({ audioOperation, track, logLevel, abortConversion, state, controller, onMediaStateUpdate, onAudioData, progressTracker }) => {
  ...
  Log3.verbose(logLevel, "Audio encoder config", audioEncoderConfig);
  Log3.verbose(logLevel, "Audio decoder config", audioDecoderConfig ?? track);
  ...
  const codecPrivate2 = audioOperation.audioCodec === "aac" ? MediaParserInternals.createAacCodecPrivate({
    audioObjectType: 2,
  ...
  const { trackNumber: trackNumber2 } = await state.addTrack({
    type: "audio",
  ...
  const audioProcessingQueue = processingQueue({
    ...
    onOutput: async (audioData) => {
      const newAudioData = onAudioData ? await onAudioData?.({
        audioData,
        track
      }) : audioData;
  ...
  state.addWaitForFinishPromise(async () => {
    Log3.verbose(logLevel, "Waiting for audio decoder to finish");
    await audioDecoder.flush();
    Log3.verbose(logLevel, "Audio decoder finished");
    audioDecoder.close();
    await audioProcessingQueue.ioSynchronizer.waitForQueueSize(0);
    Log3.verbose(logLevel, "Audio processing queue finished");
    await audioEncoder.waitForFinish();
    Log3.verbose(logLevel, "Audio encoder finished");
    audioEncoder.close();
  });
  return async (audioSample) => {
    progressTracker.setPossibleLowestTimestamp(Math.min(audioSample.timestamp, audioSample.decodingTimestamp ?? Infinity));
...
var makeAudioTrackHandler = ({ state, defaultAudioCodec: audioCodec, controller, abortConversion, onMediaStateUpdate, onAudioTrack, logLevel, outputContainer, onAudioData, progressTracker }) => async ({ track, container: inputContainer }) => {
  ...
  const audioOperation = await (onAudioTrack ?? defaultOnAudioTrackHandler)({
    defaultAudioCodec: audioCodec ?? getDefaultAudioCodec({
...
var copyVideoTrack = async ({ logLevel, state, track, onMediaStateUpdate, progressTracker }) => {
  Log3.verbose(logLevel, `Copying video track with codec ${track.codec} and timescale ${track.originalTimescale}`);
...
var defaultOnVideoTrackHandler = async ({ track, defaultVideoCodec, logLevel, rotate, canCopyTrack, resizeOperation }) => {
  if (canCopyTrack) {
    MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (video): Can copy, therefore copying`);
    return Promise.resolve({
      type: "copy"
    });
  }
  if (defaultVideoCodec === null) {
    MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (video): Is audio container, therefore dropping video`);
    return Promise.resolve({
      type: "drop"
    });
  }
  ...
  if (canReencode) {
    MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (video): Cannot copy, but re-enconde, therefore re-encoding`);
    return Promise.resolve({
      type: "reencode",
      videoCodec: defaultVideoCodec,
      rotate: void 0,
  ...
  MediaParserInternals.Log.verbose(logLevel, `Track ${track.trackId} (video): Can neither copy nor re-encode, therefore failing`);
  return Promise.resolve({
    type: "fail"
  });
};
var getDefaultVideoCodec = ({ container }) => {
  if (container === "webm") {
    return "vp8";
  }
  if (container === "mp4") {
...
var internalCreateVideoDecoder = async ({ onFrame, onError, controller, config, logLevel }) => {
  if (controller && controller._internals._mediaParserController._internals.signal.aborted) {
    throw new Error("Not creating audio decoder, already aborted");
  }
  const ioSynchronizer = makeIoSynchronizer({
...
var createVideoDecoder = ({ onFrame, onError, controller, track, logLevel }) => {
...
var needsToCorrectVideoFrame = ({ videoFrame, outputCodec }) => {
...
var convertToCorrectVideoFrame = ({ videoFrame, outputCodec }) => {
  if (!needsToCorrectVideoFrame({
    videoFrame,
    outputCodec
  })) {
    return videoFrame;
...
var processFrame = async ({ frame: unrotatedFrame, onVideoFrame, track, outputCodec, rotation, resizeOperation }) => {
  ...
  const userProcessedFrame = onVideoFrame ? await onVideoFrame({
    frame: rotated,
    track
  }) : rotated;
...
var videoFrameSorter = ({ controller, onOutput }) => {
  const frames = [];
...
var createVideoEncoder = ({ onChunk, onError, controller, config, logLevel, outputCodec, keyframeInterval }) => {
  if (controller._internals._mediaParserController._internals.signal.aborted) {
    throw new MediaParserAbortError("Not creating video encoder, already aborted");
  }
  const ioSynchronizer = makeIoSynchronizer({
    logLevel,
    label: "Video encoder",
  ...
  controller._internals._mediaParserController._internals.signal.addEventListener("abort", onAbort);
  Log3.verbose(logLevel, "Configuring video encoder", config);
  ...
  const encodeFrame = (frame) => {
    ...
    encoder.encode(convertToCorrectVideoFrame({
      videoFrame: frame,
      outputCodec
    }), {
      keyFrame,
...
var reencodeVideoTrack = async ({ videoOperation, rotate, track, logLevel, abortConversion, onMediaStateUpdate, controller, onVideoFrame, state, progressTracker }) => {
  ...
  const videoDecoderConfig = await getVideoDecoderConfigWithHardwareAcceleration(track);
  Log3.verbose(logLevel, "Video encoder config", videoEncoderConfig);
  Log3.verbose(logLevel, "Video decoder config", videoDecoderConfig ?? track);
  ...
  Log3.verbose(logLevel, `Created new video track with ID ${trackNumber2}, codec ${videoOperation.videoCodec} and timescale ${track.originalTimescale}`);
  ...
  state.addWaitForFinishPromise(async () => {
    Log3.verbose(logLevel, "Waiting for video decoder to finish");
    await videoDecoder.flush();
    videoDecoder.close();
    Log3.verbose(logLevel, "Video decoder finished. Waiting for encoder to finish");
    await frameSorter.flush();
    Log3.verbose(logLevel, "Frame sorter flushed");
    await videoProcessingQueue.ioSynchronizer.waitForQueueSize(0);
    Log3.verbose(logLevel, "Video processing queue finished");
    await videoEncoder.waitForFinish();
    videoEncoder.close();
    Log3.verbose(logLevel, "Video encoder finished");
  });
  return async (chunk) => {
    progressTracker.setPossibleLowestTimestamp(Math.min(chunk.timestamp, chunk.decodingTimestamp ?? Infinity));
    await controller._internals._mediaParserController._internals.checkForAbortAndPause();
...
var makeVideoTrackHandler = ({ state, onVideoFrame, onMediaStateUpdate, abortConversion, controller, defaultVideoCodec, onVideoTrack, logLevel, outputContainer, rotate, resizeOperation, progressTracker }) => async ({ track, container: inputContainer }) => {
  if (controller._internals._mediaParserController._internals.signal.aborted) {
    throw new Error("Aborted");
  }
  ...
  const videoOperation = await (onVideoTrack ?? defaultOnVideoTrackHandler)({
    track,
    defaultVideoCodec: defaultVideoCodec ?? getDefaultVideoCodec({
      container: outputContainer
    }),
    logLevel,
    outputContainer,
...
var throttledStateUpdate2 = ({ updateFn, everyMilliseconds, signal }) => {
  let currentState = {
...
var convertMedia = async function({ src, onVideoFrame, onAudioData, onProgress: onProgressDoNotCallDirectly, audioCodec, container, videoCodec, controller = webcodecsController(), onAudioTrack: userAudioResolver, onVideoTrack: userVideoResolver, reader, fields, logLevel = "info", writer, progressIntervalInMs, rotate, resize, onAudioCodec, onContainer, onDimensions, onDurationInSeconds, onFps, onImages, onInternalStats, onIsHdr, onKeyframes, onLocation, onMetadata, onMimeType, onName, onNumberOfAudioChannels, onRotation, onSampleRate, onSize, onSlowAudioBitrate, onSlowDurationInSeconds, onSlowFps, onSlowKeyframes, onSlowNumberOfFrames, onSlowVideoBitrate, onSlowStructure, onTracks, onUnrotatedDimensions, onVideoCodec, onM3uStreams, selectM3uStream, selectM3uAssociatedPlaylists, expectedDurationInSeconds, expectedFrameRate, seekingHints, ...more }) {
  ...
  const throttledState = throttledStateUpdate2({
  ...
  MediaParserInternals.internalParseMedia({
    ...
    onDurationInSeconds: (durationInSeconds) => {
      ...
      const expectedOutputDurationInMs = durationInSeconds * 1e3;
      throttledState.update?.((prevState) => {
        return {
          ...prevState,
