// ../node_modules/.deno/xlsx@0.18.5/node_modules/xlsx/xlsx.mjs
var XLSX = {};
XLSX.version = "0.18.5";
...
var CFB = /* @__PURE__ */ function _CFB() {
  ...
  function check_get_mver(blob) {
    if (blob[blob.l] == 80 && blob[blob.l + 1] == 75) return [
      0,
      0
    ];
...
function parse_FormulaValue(blob) {
  var b;
  if (__readUInt16LE(blob, blob.l + 6) !== 65535) return [
    parse_Xnum(blob),
    "n"
  ];
  switch (blob[blob.l]) {
...
function parse_Cache(data) {
  ...
  var nf = unescapexml((data.match(/<c:formatCode>([\s\S]*?)<\/c:formatCode>/) || [
    "",
    "General"
  ])[1]);
...
function parse_wb_bin(data, opts) {
  ...
  recordhopper(data, function hopper_wb(val, R, RT) {
    switch (RT) {
      ...
      /* 'BrtSupBookSrc' */
      case 667:
        if (!supbooks[0].length) supbooks[0] = [
          RT,
          val
        ];
        else supbooks.push([
...
// xlsx_reader.ts
async function rowsOfXlsx(src, header = 1) {
  try {
    let b = void 0;
    if (typeof src === "string") {
      const response = await fetch(src);
      b = await response.arrayBuffer();
    } else if (src instanceof File) {
      return new Promise((resolve, reject) => {
        var reader = new FileReader();
        reader.onload = async (event) => {
          resolve(await rowsOfXlsx(event.target.result, header));
        };
    ...
    alert("read xlsx failed:" + err);
  }
  return [];
}
globalThis.rowsOfXlsx = rowsOfXlsx;
export {
  rowsOfXlsx
};
